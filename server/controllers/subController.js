...
controller.buySub = async (req, res, next) => {
  try {
    const user = req.user;
    ...
    console.log(user);
    console.log(subId);
    const findUser = await User.findOne().where("_id").equals(user.id);
    findUser.purchasedSubs.push(subId)
    await findUser.save()
...
controller.updateSub = async (req, res, next) => {
  try {
    ...
    if (level > findUser.subsOwner.length)
      return next(
        APIError.BadRequests("Недопустимый уровень подписки"),
        errors.array()
      );
    ...
    if (findSub.creatorId === user.id) {
      ...
      return next(
        APIError.ForbiddenError(
          `Пользователь ${user.id} не является владельцем подписки ${subId}`
        )
      );
  } catch (e) {
    next(e);
