...
controller.likePost = async (req, res, next) => {
  try {
    ...
    findPost.countLikes++;
    findUser.likedPosts.push(findPost._id);

    await Promise.all([findPost.save(), findUser.save()]);

    return res.json({ postCount: findPost.countLikes });
  } catch (e) {
    next(e);
  }
};
...
controller.unLikePost = async (req, res, next) => {
  try {
    ...
    findPost.countLikes--;
    findUser.likedPosts.filter((id) => String(id) !== String(findPost._id));
    await Promise.all([findPost.save(), findUser.save()]);
    ...
    if (findPost.userId.toString() === req.user.id) {
      ...
      return next(
        APIError.ForbiddenError(
          `Пользователь ${req.user.id} не является создателем поста ${postId}`
        )
      );
    }
  } catch (e) {
...
controller.getAllPosts = async (req, res, next) => {
  try {
    ...
    const validate = req.headers.authorization
      ? await validateAccessToken(req.headers.authorization.split(" ")[1])
      : null;
    let allPosts;
    if (validate === null) {
...
controller.getUserPosts = async (req, res, next) => {
  try {
    const page = Number(req.params.page);
    let userPosts = await Post.find({ userName: req.params.username })
      .sort("-createdAt")
      .populate("user", "image role")
      .populate("subId", "title level");
